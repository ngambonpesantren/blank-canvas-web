...
import { cloudVaultService } from "@/core/system/vault/CloudVaultService";
import type { VaultManager } from "@/core/system/vault/VaultManager";
...
export class SyncEngine {
  ...
  async syncVault(
    ...
    if (!vault.cloudSync) {
      return { success: false, error: "Cloud sync is off for this vault" };
    }
    ...
    const snapshot: QueuedVaultPush = {
      ...
      name: vault.name,
      graph_data: vault.graphService.getGraphData() as {
        nodes: unknown[];
        links: unknown[];
      },
  ...
  private async push(
    ...
    if (!cloudId) {
      ...
      const remoteMovedAhead =
        !!remote &&
        !!base &&
        new Date(remote.updated_at).getTime() > new Date(base).getTime();

      if (remoteMovedAhead && remote) {
        const merged = this.mergeWithConflictCopies(
          remote.graph_data as {
            nodes: GraphNodeLike[];
            links: GraphLinkLike[];
          },
        ...
        payload = merged.data as unknown as {
          nodes: unknown[];
          links: unknown[];
        };
  ...
  private mergeWithConflictCopies(
        ...
        merged.push({
          ...
          id:
            typeof crypto !== "undefined" && crypto.randomUUID
              ? crypto.randomUUID()
              : `conflict-${Date.now()}-${localNode.id}`,
          name,
        });
        conflicts.push(name);
      }
    ...
    for (const l of [...(remote.links ?? []), ...(local.links ?? [])])
      links.set(linkId(l), l);
